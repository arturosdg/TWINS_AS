...
public class SucceededLevels {

    private List<Integer> succedeedLevels;
    private int id = 0;

    public SucceededLevels() {
        succedeedLevels = new ArrayList();
    }

    public SucceededLevels(int id) {
        this.id = id;
        succedeedLevels = new ArrayList();
    }
    ...
    public void loadSuccedeedLevels(SharedPreferences sp) {
        ...
        if (sp.getString("LEVEL" + id, null) != null) {
            Gson gson = new Gson();
            SucceededLevels levels = gson.fromJson(sp.getString("LEVEL" + id, null), SucceededLevels.class);
